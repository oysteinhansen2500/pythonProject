...
def customerGenerator(env, employees):
...
def customer(env, id):
     ...
     #print("fetching mos", shoppingList, items, queueTime)
...
def runSim(employees, simCount):
     global containers,  MOS_scores, stationResources
     MOS_averages = []
     ...
     for i in range(simCount):
          ...
          stationCheckout = containers.append(simpy.Resource(env, capacity=8))
          #print(containers)
          #print(containers[1].capacity)

          env.process(customerGenerator(env, employees))
          env.run(until=960)
          #print("mos scores", MOS_scores)
          MOS_averages.append(sum(MOS_scores) / len(MOS_scores))
          #print("mos averages", MOS_averages)
     return MOS_averages
...
for x in range(10):
     #print("rooning")
     average_mos.append(runSim(x+1, 100))
     ##print(average_mos)
